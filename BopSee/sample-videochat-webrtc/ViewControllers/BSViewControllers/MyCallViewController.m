...
@interface MyCallViewController ()<UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, QBRTCClientDelegate, LocalVideoViewDelegate>
...
@property (weak, nonatomic) IBOutlet UILabel *lblCallName;
...
- (void)viewDidLoad {
    ...
    if (self.session.conferenceType == QBRTCConferenceTypeVideo) {
        ...
        self.cameraCapture = [[QBRTCCameraCapture alloc] initWithVideoFormat:settings.videoFormat
                                                                    position:2]; //settings.preferredCameraPostion];   --->AVCaptureDevicePositionFront
    ...
    _lblCallName.text = [appController.callUser objectForKey:@"user_name"];
}
...
- (void)configureGUI {
    ...
    _btnAbuse.layer.cornerRadius = 10.0f;
...
- (IBAction)onClickedAbuse:(id)sender {
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    ...
    return 1; //self.users.count;
}

- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    
    OpponentCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"OpponentCollectionViewCellIdentifier"
                                                                                 forIndexPath:indexPath];
    QBUUser *user = self.users[1];//indexPath.row];
    
    [cell setVideoView:[self videoViewWithOpponentID:@(user.ID)]];
    
    //My custom
//    if(indexPath.row == 0){
//        [cell setColorMarkerText:[appController.currentUser objectForKey:@"user_name"] andColor:user.color];
//    }
//    else{
//        [cell setColorMarkerText:[appController.callUser objectForKey:@"user_name"] andColor:user.color];
//    }
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGRect frame = [OpponentsFlowLayout frameForWithNumberOfItems:1 //self.users.count
                                                              row:indexPath.row
...
- (NSIndexPath *)indexPathAtUserID:(NSNumber *)userID {
    ...
    NSUInteger idx = 0;//[self.users indexOfObject:user];
...
- (void)session:(QBRTCSession *)session userDoesNotRespond:(NSNumber *)userID {
    ...
    NSLog(@"you are calling to user, but he hasn't answered");
}
...
- (void)session:(QBRTCSession *)session rejectedByUser:(NSNumber *)userID userInfo:(NSDictionary *)userInfo {
    ...
    NSLog(@"-----opponent has rejected you call");
}
...
- (void)session:(QBRTCSession *)session disconnectedFromUser:(NSNumber *)userID {
    ...
    NSLog(@"--------disconnected from opponent");
}
...
- (void)session:(QBRTCSession *)session connectionFailedWithUser:(NSNumber *)userID {
    ...
    NSLog(@"---------connection failed with user");
}
...
//        }
        ...
        //[commonUtils showAlert:@"Warning" withMessage:@"session close"];
