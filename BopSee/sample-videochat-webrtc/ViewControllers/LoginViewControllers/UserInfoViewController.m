...
-(void)viewWillAppear:(BOOL)animated{
    if (appController.sel_Cancel != 0) {
        userLocation = appController.selectAddress;
...
- (void) initView{
    ...
    if([[appController.currentUser objectForKey:@"user_name"] isEqualToString:@""] || [appController.currentUser objectForKey:@"user_name"] == nil){
        NSArray *array = [[appController.currentUser objectForKey:@"user_full_name"] componentsSeparatedByString:@" "];
        if ([array objectAtIndex:0] != nil) {
            _txtDisplayName.text = [array objectAtIndex:0];
        }
    ...
    userLocation = [appController.currentUser objectForKey:@"user_address"];
...
- (void) searchCurrentAddress{
    ...
    [geocoder reverseGeocodeLocation:newLocation completionHandler:^(NSArray *placemarks, NSError *error) {
        ...
        if (error == nil) {
            ...
            userLocation = [NSString stringWithFormat:@"%@, %@", userSpotPlaceMark.locality, userSpotPlaceMark.administrativeArea];
            _lblLocation.text = userLocation;
            [JSWaiter HideWaiter];
