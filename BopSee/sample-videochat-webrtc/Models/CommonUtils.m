...
- (void) setCircleViewImage: (UIView*)view imageview:(UIImageView*)imageview borderWidth:(float)border borderColor:(UIColor*) borderColor{
    
    CGFloat radius = MAX(view.bounds.size.height, view.bounds.size.height);
    CGPoint center = view.center;
    view.frame = CGRectMake(center.x - radius/2
                                      , center.y - radius/2
                                      , radius, radius);
...
- (NSString *)convertTimeFormat:(NSString*)dateString{
    
    NSDateFormatter *format = [[NSDateFormatter alloc] init];
    [format setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSDate *date = [format dateFromString:dateString];
    [format setDateFormat:@"MMMM dd, hh:mma"];
    NSString* finalDateString = [format stringFromDate:date];
    return finalDateString;

}
-(void)QBUserbyID:(NSUInteger)ID
{
    [QBRequest userWithID:ID successBlock:^(QBResponse *response, QBUUser *user) {
        // Successful response with user
        NSLog(@"QB User : %@", user);
        [[NSNotificationCenter defaultCenter] postNotificationName:@"QBUser" object:nil];
...
-(void)QBSignup:(NSString*)userEmail
{
    ...
    [QBRequest signUp:user successBlock:^(QBResponse *response, QBUUser *user) {
        [QBRequest logInWithUserLogin:user.login password:password successBlock:^(QBResponse *response, QBUUser *user) {
            //add new User to UsersDataSource
            //user.password = DEFAULT_QBPASSWORD;
            //[UsersDataSource.instance addUser:user];
            
            NSNumber *qb_id = [NSNumber numberWithUnsignedInteger:user.ID];
            [commonUtils setUserDefault:@"current_user_qb_id" withFormat:[NSString stringWithFormat:@"%ld", (long)qb_id.integerValue]];
            NSLog(@"QB Signup Success with : %@", user);
       
            [[NSNotificationCenter defaultCenter] postNotificationName:APP_QBSignUPSuccess object:nil];
...
//QBVideoChat in Dashboard!!

-(void)QBlogout
{
    [ChatManager.instance logOut];
    [QBRequest logOutWithSuccessBlock:^(QBResponse *response) {
        NSLog(@"Logout Success with QB!");
    } errorBlock:^(QBResponse *response) {
